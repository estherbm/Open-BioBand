...
#define LEDPIN3 5         //LED brightness (PWM) writing
...
void loop()
{
  ...
  // light.manualStart();
   ...
   //float square_ratio1 = LDRReading /1023;      //Get percent of maximum value (1023)
   //square_ratio1 = pow(square_ratio1, 2.0);      //Square to make response more obvious
   int square_ratio1= map(LDRReading, 0,1023,70,0);
   if (square_ratio1<50) {analogWrite(LEDPIN1, 0); } 
   else {analogWrite(LEDPIN1, 255); } //Adjust LED brightness relatively
  
  
  float reading = analogRead(LIGHTSENSORPIN); //Read light level
  float square_ratio2 = reading / 1023.0;      //Get percent of maximum value (1023)
  square_ratio2 = pow(square_ratio2, 2.0);      //Square to make response more obvious
  analogWrite(LEDPIN3, 255.0 * square_ratio2);  //Adjust LED brightness relatively
  ...
  if (light.getData(data0,data1))
  {
   ...
   Serial.print(" data1: ");
    ...
    Serial.print("LDR: ");
    Serial.println(square_ratio1);
    Serial.print("TEMT6000: ");
    Serial.println(255*square_ratio2);  
    int data2=map(data1,20,200,0,80);
    analogWrite(LEDPIN2, data2);  //Adjust LED brightness relatively
    Serial.print(" data1: ");
